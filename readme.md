...
This is needed to import images into CRIO scope, as CRIO does not natively support image importation. To setup the local registry, take the steps below:
...
This is needed in a case where Server **A** hosts a private image registry, and has access to another registry (e.g ACR) over the internet. And Server **B** has access to Server **A's** private image registry but not a public registry like ACR.
